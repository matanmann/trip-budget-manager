...
async function init() {
    try {
        ...
        // Only show login for actual 401 auth errors
        if (error.message.includes('401') || error.message.includes('Not authenticated')) {
            showLogin();
        } else {
            // Something else went wrong — show login but also surface the real error
            showLogin();
...
function renderTabContent() {
    ...
    switch (state.currentTab) {
        ...
        case 'add':       renderAddExpenseForm(); break;
        case 'settings':  renderTripSettings();  break;
    }
}

function renderDashboard() {
    const trip = state.activeTrip;
    // Always calculate spent from local expenses — never trust trip.spent from server
    const spent = state.expenses.reduce((sum, e) => sum + e.amount, 0);
    const remaining = trip.totalBudget - spent;
    const percentage = trip.totalBudget > 0 ? (spent / trip.totalBudget) * 100 : 0;
...
window.deleteTrip = async function(tripId) {
    ...
    try {
        ...
        state.expenses = state.activeTrip
            ? await api.getExpenses(state.activeTrip.id)
            : [];
